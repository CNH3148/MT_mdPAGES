...
**Quota 額度管理（三帳號智能輪替）**：
- 透過 `AccountManager` 類別統一管理三個 Google 帳號的輪替與額度追蹤
- 偵測到額度耗盡時，自動解析畫面上的恢復時間，記錄至該帳號，然後切換至下一個有額度的帳號繼續執行
- 每次 Alt+G 重開彈窗（含定期重開與錯誤復原）後，自動檢查帳號是否被意外切換，若被換掉則切回原帳號，確保耗盡當前帳號額度後再換下一個
